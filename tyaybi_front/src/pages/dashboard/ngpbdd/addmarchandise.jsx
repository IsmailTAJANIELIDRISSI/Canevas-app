...
import { postngp } from "@/services/ngpservice"; // Adjust import path as per your setup
...
export function Addform(props) {
  ...
  const handleChange = (e, name) => {
    ...
    setFormData({
      ...formData,
      [name]: name === 'codeNGP' && value !== '' ? parseInt(value) : value
    });
  ...
  const validateForm = () => {
    ...
    if (!formData.codeNGP) {
      ...
      isValid = false;
    } else if (!/^\d{10}$/.test(formData.codeNGP)) {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      if (confirmed) {
        try {
          await postngp(formData);
          swal("Bravo", "Marchandise ajoutée avec succès.", { icon: "success" });
          props.setReload(prev => !prev); // Reload data after successful submission
          props.handleOpen(); // Close the dialog
          console.log("NGP added successfully!");
        } catch (error) {
          console.error("Error adding NGP:", error);
          swal("Erreur", "Une erreur s'est produite lors de l'ajout du Marchandise.", { icon: "error" });
        }
      } else {
        ...
        <Card className="mx-auto ps-10 w-full max-w-[33rem]">
          ...
          <CardBody className="flex flex-col gap-4">
            ...
            <Typography
              ...
              color="gray"
            >
              Entrer les détails du Marchandise
            </Typography>
